...
def format_due(task):
        """Taskning muddatini chiroyli ko'rsatish"""
        due = task.get("due")
        if not due:
                    return ""
    ...
    if not due_date_str:
            ...
            try:
                        ...
                        if delta < 0:
                                        return f" <b>(muddat: {formatted}, {abs(delta)} kun o'tdi!)</b>"
elif delta == 0:
            return f" <b>(muddat: bugun!)</b>"
elif delta == 1:
            return f" (muddat: ertaga, {formatted})"
else:
            return f" (muddat: {formatted})"
        except:
                    return f" (muddat: {due_date_str[:10]})"
...
def send_today_report():
        """Bugun tugashi kerak bo'lgan ishlarni alohida hisobot qilib yuborish"""
    today = date.today().strftime("%Y-%m-%d")
    message = f"<b>BUGUNGI MUDDAT | {date.today().strftime('%d.%m.%Y')}</b>\n"
    message += "________________________\n\n"
    has_tasks = False
    for name, section_id in SECTIONS.items():
                tasks = get_tasks(section_id)
        ...
        for task in tasks:
                        due = task.get("due")
                        if due and due.get("date", "")[:10] == today:
    ...
    if has_tasks:
                send_message(message)
        print(f"Bugungi hisobot yuborildi: {datetime.now().strftime('%H:%M')}")
else:
        print("Bugun tugashi kerak bo'lgan vazifalar yo'q")
    ...
    while True:
                try:
                                ...
                                for update in updates:
                                                    ...
                                                    # "?" belgisi kelsa - joriy hisobotni qayta yuborish
                                                    if text == "?":
                                                                            build_and_send()
                                                                            continue

                                                    # Faqat + bilan boshlangan xabarlar - task qo'shish
...
# Bugungi muddat hisoboti - har kuni ertalab 08:00 da
schedule.every().day.at("08:00").do(send_today_report)
