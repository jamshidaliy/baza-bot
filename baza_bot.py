...
async def send_jobir_reminder():
            ...
            if now.weekday() in [1, 3, 5]:
                            await bot.send_message(chat_id=CHAT_ID, message_thread_id=MULOQOT_THREAD_ID, text=JOBIR_MSG)
                            logger.info(f"Jobir eslatmasi yuborildi: {now}")

        async def send_jamshid_reminder():
                    now = datetime.now(TASHKENT_TZ)
                    if now.weekday() in [0, 2, 4]:
                                    await bot.send_message(chat_id=CHAT_ID, message_thread_id=MULOQOT_THREAD_ID, text=JAMSHID_MSG)
